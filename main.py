...
def process():
    ...
    with engine.connect() as conn:
        ...
        # Processaremos as duas listas, mas com um limite estrito para não travar
        for list_id, handler in [("141", "lp1"), ("144", "lp2")]:
            print(f"Buscando novos contatos da {handler}...")
            
            # Buscamos apenas a primeira página (mais recentes)
            url = "https://campuscaldeira.pipz.io/api/v1/contact/"
            params = {
                "list_id": list_id, 
                "limit": 100, # Processa 100 por vez conforme sua ideia
                "api_key": PIPZ_KEY, 
            ...
            if res.status_code != 200:
                print(f"Erro ao acessar lista {handler}: {res.status_code}")
                continue
            ...
            for summary in batch:
                try:
                    ...
                    if not detail:
                        print(f"[AVISO] ID {summary['id']} não retornou detalhes no Pipz.")
                        continue
                    ...
                    with conn.begin():
                        ...
                        if handler == "lp1":
                            ...
                            """), {"p_id": pessoa_id, "est": f.get('state'), "cid": f.get('city_name'), "sab": sab, "cod": f.get("gc_2026_codigo_alumni"), "dt": format_timestamp(f.get('creation_date'))})
                    ...
                    # LOG DE ERRO DETALHADO POR USUÁRIO
                    print(f"[ERRO] Usuário {summary.get('id')} não processado. Motivo: {str(e)[:150]}")
