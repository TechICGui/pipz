...
def normalize_etnia(f):
    """Busca etnia conforme os nomes técnicos exatos fornecidos"""
    raw = (f.get("[GC 2026 LP2] qual etnia") or f.get("[GC 2026 LP2] etnia") or 
           f.get("gc_2026_lp2_etnia") or f.get("[2025] ETNIA") or f.get("etnia") or "")
    txt = str(raw).lower()
    ...
    with engine.connect() as conn:
        ...
        for list_id, handler in [("141", "lp1"), ("144", "lp2")]:
            ...
            for summary in summary_list:
                ...
                # Transação individual por pessoa
                with conn.begin():
                    try:
                        conn.execute(text("""
                        ...
                        if handler == "lp1":
                            alumni = f.get("gc2026_codigo_alumni") or f.get("[GC2026] codigo alumni") or f.get("[2025] CUPOM GC 2025")
                            # Nome EXATO do rótulo que você enviou
                            sabendo = f.get("[2025] Como ficou sabendo do Geração Caldeira?") or f.get("[GC 2026 LP1] Origem") or f.get("gc_2026_lp1_origem")
                        ...
                        if handler == "lp2":
                            genero = normalize_genero(f.get("[GC 2026 LP2] Gênero") or f.get("[GC 2026] Genero") or f.get("gender"))
                            etnia = normalize_etnia(f)
                            trabalha = "Sim" if "sim" in str(f.get("[GC 2026 LP2] você trabalha") or "").lower() else "Não"
                            ...
                            """), {
                                "cpf": final_cpf, "tri": f.get("[GC 2026 LP2] trilha educacional"),
